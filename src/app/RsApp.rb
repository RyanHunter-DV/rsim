...
class Application
	...
	def run
		...
		#if @ui.chain[:name]=='sim'
		#	parse_test_options if @ui.chain[:params].has_key?('--test')
		#end
	...
	def chain_execute(chain)
		...
		chain[:params]['--remote'] = RsApp.ui.options[:remote] if RsApp.ui.options[:remote]
		RsApp.chains[chain[:name]].execute(chain[:params]);
	end

	# api to read from the test.xml file and get required information
